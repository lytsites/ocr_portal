    ...
    <main class="content">
        ...
        <div class="filters-grid filters-grid-wide">
          ...
          <label class="field">
            ...
            <select v-model="selectedOked">
              <option value="">Все</option>
              <option v-for="x in filters.okeds" :key="x" :value="x">{{ x }}</option>
            </select>
          </label>
          <label class="field">
      ...
      <section v-if="analyticsTab === 'kpi_top'" class="kpi-grid kpi-grid-wide">
        ...
        <article class="card kpi kpi-2">
          ...
          <div class="kpi-value" :class="{ positive: (kpi.yoy_percent ?? 0) >= 0, negative: (kpi.yoy_percent ?? 0) < 0 }">{{ percent(kpi.yoy_percent) }}</div>
          <div class="kpi-sub">Прошлый год: {{ money(previousTotalAmount) }}</div>
        </article>
          ...
          <div class="taxpayer-top-grid">
            ...
            <article class="card profile-card compact">
              ...
              <div class="profile-pairs">
                ...
                <div><span>YoY:</span><strong :class="{ positive: (effectiveTaxpayerProfile.yoy_percent ?? 0) >= 0, negative: (effectiveTaxpayerProfile.yoy_percent ?? 0) < 0 }">{{ percent(effectiveTaxpayerProfile.yoy_percent) }}</strong></div>
              </div>
            </article>
...
const analyticsUrl = String(import.meta.env.VITE_ANALYTICS_URL || "https://analytics.e-qoldau.asia");
const SITE_LINKS = [
  { id: "form_2_19", label: "2-19", url: String(import.meta.env.VITE_SITE_URL_2_19 || "https://219.e-qoldau.asia") },
  { id: "form_2_43", label: "2-43", url: String(import.meta.env.VITE_SITE_URL_2_43 || "https://243.e-qoldau.asia") },
  { id: "form_4_20", label: "4-20", url: String(import.meta.env.VITE_SITE_URL_4_20 || "https://420.e-qoldau.asia") },
  { id: "form_5_52", label: "5-52", url: String(import.meta.env.VITE_SITE_URL_5_52 || "https://552.e-qoldau.asia") },
];
...
const filters = ref({ regions: [], periods: [], kbks: [], banks: [], categories: [], okeds: [], msb_segments: [], periods_by_region: {}, kbks_by_region: {}, kbks_by_region_period: {} });
...
const kpi = ref({ total_amount: 0, yoy_percent: null, unique_taxpayers: 0, top10_share_percent: 0, msb_share_percent: 0, cr10_percent: 0 });
...
const previousTotalAmount = computed(() => {
  const yoy = Number(kpi.value?.yoy_percent);
  const total = Number(kpi.value?.total_amount || 0);
  if (!Number.isFinite(yoy)) return 0;
...
function zeroState() {
  kpi.value = { total_amount: 0, yoy_percent: null, unique_taxpayers: 0, top10_share_percent: 0, msb_share_percent: 0, cr10_percent: 0 };
  summaryByKbk.value = [];
  topAnalytics.value = [];
  msbBreakdown.value = [];
  msbTop10.value = [];
...
watch(filters, () => {
  ...
  if (selectedOked.value && !filters.value.okeds.includes(selectedOked.value)) selectedOked.value = "";
